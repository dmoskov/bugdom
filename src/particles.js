...
// ============================================
// GRAVITOID PARTICLES
// Special particles with mutual gravity
// ============================================

export class GravitoidSystem {
    constructor(scene) {
        this.scene = scene;
        this.particles = [];
        this.maxParticles = 100;
        this.gravityStrength = 5.0;
    }

    addGravitoid(position, velocity, color = 0xff00ff, size = 0.1) {
        if (this.particles.length >= this.maxParticles) {
            return null;
        }
    ...
    update(deltaTime) {
        // Apply mutual gravity between particles
        for (let i = 0; i < this.particles.length; i++) {
            for (let j = i + 1; j < this.particles.length; j++) {
                ...
                direction.subVectors(p2.mesh.position, p1.mesh.position);
                ...
                if (distance > 0.1) {
                    ...
                    p1.velocity.add(direction.multiplyScalar(force * deltaTime / p1.mass));
                    p2.velocity.sub(direction.multiplyScalar(force * deltaTime / p2.mass));
                }
            }
        }
